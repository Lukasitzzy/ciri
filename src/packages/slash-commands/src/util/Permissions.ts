...
export class CustomPermissions extends Permissions {
    ...
    public static resolve(bit: keyof typeof PermissionStrings | bigint | BitField<keyof typeof PermissionStrings, bigint>): bigint {
        //@ts-ignore
        const { defaultBit } = this;
        if (typeof bit === 'undefined') return defaultBit;
        if (typeof defaultBit === typeof bit && bit >= defaultBit) return BigInt(bit);
        ...
        if (bit === 'USE_APPLICATION_COMMANDS') return PermissionStrings.USE_APPLICATION_COMMANDS;
        if (typeof bit === 'string' && typeof (this.FLAGS[bit as keyof typeof PermissionStrings]) !== 'undefined') return this.FLAGS[bit as keyof typeof PermissionStrings];
        //@
