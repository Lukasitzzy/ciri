...
export class InterActionCommand extends InteractionBase {
    ...
    public _parse(
        ...
        if (member) {
            ...
            this._user = this.client.users.cache.get(member) as AitherUser | undefined;
        }
        if (user) {
            this._user = this.client.users.cache.get(user) as AitherUser | undefined;
        }
    ...
    private async _reply({
        ...
        if (options?.ephemeral) {
            ...
            data.data.flags = 64;
            data.type = type || InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE;
            if (options.options?.embeds) {

                data.data.embeds = options.options.embeds.map(e => e instanceof MessageEmbed ? e.toJSON() :e ) as Record<string, unknown>[]; 
            }
            data.data.type = type || InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE;
            try {
    ...
    // private _sendWebhook(content: string, options?: WebhookMessageOptions) {
    //     getApi(this.client).interactions(this.id, this.token).
    // }
...
export interface GuildInteractionCommand extends InterActionCommand {
    user: AitherUser;
    guild: AitherGuild;
    member: GuildMember;
    channel: TextChannel;
...
export interface DmInteractionCommand extends InterActionCommand {
    user: AitherUser;
    guild: undefined;
    member: undefined;
    channel: DMChannel;
