...
export default class PingCommand extends CustomCommand {
    ...
    public async run(ctx: CommandContext<Record<string, unknown>>): Promise<void> {
        ...
        await pingMessage.edit(`PONG! \`${clientWSPing}\`ms ( message \`${(pingMessage.editedTimestamp || pingMessage.createdTimestamp) - (ctx.msg.editedTimestamp || ctx.createdTimestamp)}\`ms )`);
    }
